...
def benchmark_source(MC, state_space,
                     size_smallest_subsample, size_largest_subsample, step_of_subsampling,
                     adress_results,
                     LIST_METHODS):
    """Benchmarking of the methods on the given Markov chain, plotting and saving by subsampling
    :markovchain: MC:markov chain to be analysed
    :int: size_smallest_subsample: smallest_subsample to be taken into account when benchmarking
    :int: size_largest_subsample: largest_subsample to be taken into account when benchmarking
    :int: step_of_subsampling: step by which the all subsamples will increase with respect to the previous subsample in the benchmarking
    :string: adress_results where to store some results of the benchmarking, in case the plots don't show enough information
    ...
    for method in LIST_METHODS:
        ...
        for n in N_range:
            ...
            for state in MC.states[:n]:
                smaller_mc.next_state(state)
...
def benchmark_dynamic(MC, state_space,
                     size_smallest_subsample, size_largest_subsample, step_of_subsampling,
                     adress_results,
                     LIST_METHODS):
    if size_smallest_subsample == 1:
